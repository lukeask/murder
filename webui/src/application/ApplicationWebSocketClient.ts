...
export function defaultApplicationWebSocketUrl(): string {
...
interface ProjectionRegistration {
  ...
  ready: boolean;
  tailBuffer: Array<{ cursor: number | null; invalidation: ProjectionInvalidation }>;
...
export class ApplicationWebSocketClient implements ApplicationClient {
  ...
  constructor(options: ApplicationWebSocketClientOptions = {}) {
    this.url = options.url ?? defaultApplicationWebSocketUrl();
  ...
  private acceptProjectionReady(id: string, snapshot: SubscriptionSnapshot): void {
    ...
    for (const buffered of projection.tailBuffer) {
      this.deliverProjectionPayload(projection, buffered.cursor, buffered.invalidation);
    }
  ...
  private acceptProjectionEvent(
    ...
    if (!projection.ready) {
      if (isProjectionInvalidation(payload)) {
        projection.tailBuffer.push({ cursor: cursor ?? null, invalidation: payload });
      }
      return;
    }
