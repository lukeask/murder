...
# ---------------------------------------------------------------------------
# _color_for_pct — lower usage is greener
# ---------------------------------------------------------------------------


def test_color_green_low_usage() -> None:
    assert _color_for_pct(0.0) == "green"
    assert _color_for_pct(39.9) == "green"


def test_color_yellow_moderate_usage() -> None:
    assert _color_for_pct(40.0) == "yellow"
    assert _color_for_pct(74.9) == "yellow"


def test_color_red_high_usage() -> None:
    assert _color_for_pct(75.0) == "red"
...
def test_period_minutes_for_uses_window_name() -> None:
    assert _period_minutes_for("codex", "5h", {}) == 5 * 60.0
    assert _period_minutes_for("anything", "7d", {}) == 7 * 24 * 60.0


def test_period_minutes_for_falls_back_to_constants_table() -> None:
    assert _period_minutes_for("codex", "weekly", {}) == 7 * 24 * 60.0
    assert _period_minutes_for("claude_code", "current_session", {}) == 5 * 60.0


def test_period_minutes_for_unknown_window_is_zero() -> None:
    assert _period_minutes_for("mystery", "whatever", {}) == 0.0
...
def test_render_gauge_contains_window_key() -> None:
...
def test_render_gauge_contains_pct() -> None:
    g = _GaugeData(harness="cursor", window_key="api", pct=73.0, t_until_reset_minutes=30.0)
    rendered = _render_gauge(g, focused=False)
    assert "73%" in rendered


def test_render_gauge_contains_reset() -> None:
    g = _GaugeData(harness="cursor", window_key="api", pct=50.0, t_until_reset_minutes=90.0)
    rendered = _render_gauge(g, focused=False)
    assert "rst" in rendered
    assert "1h30m" in rendered


def test_render_gauge_shows_period_alongside_reset() -> None:
    g = _GaugeData(
        harness="codex",
        window_key="weekly",
        pct=40.0,
    ...
    rendered = _render_gauge(g, focused=False)
    assert "1h30m/5h" in rendered


def test_render_gauge_green_on_low_usage() -> None:
    g = _GaugeData(harness="cursor", window_key="api", pct=10.0, t_until_reset_minutes=60.0)
    rendered = _render_gauge(g, focused=False)
    assert "[green]" in rendered


def test_render_gauge_red_on_high_usage() -> None:
    g = _GaugeData(harness="cursor", window_key="api", pct=85.0, t_until_reset_minutes=60.0)
    rendered = _render_gauge(g, focused=False)
...
def test_load_gauges_orders_by_provider() -> None:
    db = _make_db()
    _insert_snapshot(db, "cursor", 8.0, "api")
    _insert_snapshot(db, "claude_code", 0.0, "current_session")
    _insert_snapshot(db, "codex", 3.0, "5h")
    gauges = _load_gauges(db)
    assert [g.harness for g in gauges] == ["claude_code", "codex", "cursor"]


# ---------------------------------------------------------------------------
# _build_strip_text — provider columns
# ---------------------------------------------------------------------------


def test_build_strip_text_empty() -> None:
    assert _build_strip_text([], 0) == ""


def test_build_strip_text_groups_into_provider_columns() -> None:
    gauges = [
        _GaugeData("claude_code", "current_session", 0.0, 30.0),
        _GaugeData("codex", "5h", 3.0, 200.0),
        _GaugeData("codex", "weekly", 74.0, 2600.0),
        _GaugeData("cursor", "api", 8.0, 4000.0),
    ]
    lines = _build_strip_text(gauges, focus_idx=0).split("\n")
    # header row + 2 gauge rows (codex has the most windows: 2)
    assert len(lines) == 3
    assert "claude code" in lines[0]
    assert "codex" in lines[0]
    assert "cursor" in lines[0]
    # first window of each provider on row 1, codex's second window on row 2
    assert "current_session" in lines[1]
    assert "weekly" in lines[2]


def test_build_strip_text_marks_focused_gauge() -> None:
    gauges = [
        _GaugeData("codex", "5h", 3.0, 200.0),
        _GaugeData("codex", "weekly", 74.0, 2600.0),
    ]
    assert "[b]\\[[/b]" in _build_strip_text(gauges, focus_idx=1)
