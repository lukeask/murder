...
agrees on the three things that matter for dispatch correctness:
  ...
  - ``PROTOCOL_VERSION`` (both must be 3; a client refuses a mismatched server).
...
def test_protocol_version_agrees(ts_source: str) -> None:
    assert pyproto.PROTOCOL_VERSION == 3, "Python PROTOCOL_VERSION drifted from expected 3"
...
def test_entity_enum_value_set_agrees(ts_source: str) -> None:
    ...
    assert len(py) == 7, f"expected 7 Entity values in Python, got {sorted(py)}"
