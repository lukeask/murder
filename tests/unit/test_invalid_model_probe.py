...
async def test_invalid_model_probe_sends_slash_model_and_detects_rejection(
    ...
    result = await adapter.attach("sess", Path("/repo")).probe_invalid_model("thisisnotarealmodel")
...
async def test_invalid_model_probe_fails_when_pane_has_no_rejection(monkeypatch, adapter) -> None:
    async def fake_send_keys(
        session: str, text: str, *, literal: bool = True, enter: bool = True
    ) -> None:
