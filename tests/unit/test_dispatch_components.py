...
def test_dispatch_store_attention_counts_empty() -> None:
    ...
    store = DispatchStore()
    store.ingest_snapshot(factory_dispatch_snapshot(()))
...
def test_schedule_store_sorted_rows_empty() -> None:
    ...
    store.ingest_snapshot(factory_schedule_snapshot())
    snap = store.get_snapshot()
...
def test_schedule_store_sorted_rows_order() -> None:
    ...
    t1 = factory_schedule_ticket_row("t001", last_update_at=_DT1)
    t2 = factory_schedule_ticket_row("t002", last_update_at=_DT2)
    store = ScheduleStore(FakeAsyncLoader())
    store.ingest_snapshot(factory_schedule_snapshot(active_tickets=(t1, t2)))
    snap = store.get_snapshot()
...
def test_header_refresh_from_snapshot_bridge_compat() -> None:
    ...
    snap = factory_dispatch_snapshot(())
...
def test_schedule_tickets_table_subscribes_and_unsubscribes() -> None:
    ...
    store.ingest_snapshot(factory_schedule_snapshot(invalidation_key="k2"))
    assert len(widget.rendered) == initial + 1

    widget.on_unmount()
    widget.rendered.clear()
    store.ingest_snapshot(factory_schedule_snapshot(invalidation_key="k3"))
...
def test_sort_schedule_rows_single() -> None:
    row = factory_schedule_ticket_row("t001")
