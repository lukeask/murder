...
describe('submitCommand', () => {
  ...
  it('re-polls command.status through running/queued until a terminal status', async () => {
    ...
    const statusCalls = fake.rpcCalls.filter((c) => c.method === 'command.status');
  ...
  it('rejects with a timeout after MAX_POLLS non-terminal polls', async () => {
    ...
    const fake = new FakeBusClient();
    fake.stubRpc('command.submit', { ok: true, command_id: 'cmd-1' });
    fake.stubRpc('command.status', { ok: true, status: 'running' });
