/**
 * CrowsSurface — store-free, dimension-driven crows list (new pane contract).
 *
 ...
 * picks the display mode. Matches {@link ../CrowsSurface.tsx} list intent at large sizes.
 */
...
export type CrowsSurfaceStatus = 'idle' | 'loading' | 'error';

export interface CrowsSurfaceRow {
  readonly id: string;
...
export interface CrowsSurfaceProps {
...
type CrowLedgerRow =
  ...
  | { readonly kind: 'crow'; readonly row: CrowsSurfaceRow };
...
function countSectionHeaders(rows: readonly CrowsSurfaceRow[]): number {
  let count = 0;
  let group = '';
...
export const CrowsSurface = memo(function CrowsSurface({
  width,
  height,
  focused,
  theme,
