/**
 * UsagePanel — store-free, dimension-driven usage gauges for fixtures and the new pane contract.
 *
...
export interface UsagePanelGauge {
  readonly label: string;
  readonly pct: number;
  readonly reset: string;
}

export interface UsagePanelGroup {
  readonly harness: string;
  readonly steering: string;
  readonly gauges: readonly UsagePanelGauge[];
}
...
export interface UsagePanelProps {
  ...
  readonly groups: readonly UsagePanelGroup[];
...
function longestLabelInGroup(gauges: readonly UsagePanelGauge[]): number {
  return gauges.reduce((max, gauge) => Math.max(max, gauge.label.length), 0);
}

function keyLabelWidthForGroups(
  groups: readonly UsagePanelGroup[],
...
function toGaugeGeometry(gauge: UsagePanelGauge): GaugeGeometry {
...
function flattenGauges(groups: readonly UsagePanelGroup[]): readonly FlatGauge[] {
...
function countGauges(groups: readonly UsagePanelGroup[]): number {
...
function shouldSpreadSingleGroup(
  innerW: number,
  innerH: number,
  gaugeCount: number,
): boolean {
...
function spreadColumnsForSingleGroup(group: UsagePanelGroup): readonly UsagePanelGroup[] {
...
function SpreadGroupColumn({
  ...
  readonly group: UsagePanelGroup;
  readonly width: number;
  readonly cursor: number;
  readonly focused: boolean;
  readonly padding: UsageLayoutOptions;
  ...
  return (
    <Box flexDirection="column" flexShrink={0} width={width}>
      <HeaderLine harness={group.harness} steering={group.steering} compact={headerCompact} />
      {gauges.map((gauge) => {
        gaugeIndex += 1;
        return (
          <GaugeLine
...
function UsageBody({
  ...
  if (displayMode === 'tiny') {
    ...
    return (
      <Box flexDirection="column" flexShrink={0}>
        <HeaderLine harness={group?.harness ?? '?'} steering={group?.steering ?? 'auto'} compact />
        <GaugeLine
          gauge={gauge.geometry}
          selected={focused}
          layout={gaugeLineLayout}
  ...
  if (spreadGroups || spreadSingleGroup) {
    const spreadSourceGroups = spreadSingleGroup
      ? spreadColumnsForSingleGroup(groups[0] as UsagePanelGroup)
      : groups;
    return (
      <Box flexDirection="column" flexShrink={0} height={innerH} overflow="hidden">
        {renderSpreadGroups({
    ...
    <Box flexDirection="column" flexShrink={0} height={innerH} overflow="hidden">
      ...
      {visibleGroups.map((group) => (
        <Box key={group.harness} flexDirection="column" flexShrink={0}>
          <HeaderLine harness={group.harness} steering={group.steering} compact={compactHeader} />
