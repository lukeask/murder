/**
 * HistorySurface — explicit width/height pane contract for the user-intention history feed.
 *
 * Store-free: callers pass display-ready rows and chrome props. Matches the old
 * {@link ../HistorySurface.tsx} list layout at large sizes; smaller allocations route through
...
export type HistorySurfaceMode = 'loose' | 'all';

export type HistorySurfaceStatus = 'idle' | 'loading' | 'error';

export interface HistorySurfaceRow {
  readonly id: string;
  readonly age: string;
...
export interface HistorySurfaceProps {
  ...
  readonly rows: readonly HistorySurfaceRow[];
  /** Loose-thread filter — OPEN+STALE only when `loose` (default). */
  readonly mode?: HistorySurfaceMode;
  readonly cursor?: number;
  readonly status?: HistorySurfaceStatus;
...
function looseCount(rows: readonly HistorySurfaceRow[]): number {
...
export function emptyStateText(mode: HistorySurfaceMode, innerW: number): string {
...
export const HistorySurface = memo(function HistorySurface({
  ...
  status = 'idle',
  error = null,
}: HistorySurfaceProps): React.JSX.Element {
