/**
 * NotesPanel — store-free, dimension-driven notes list for fixtures and the new pane contract.
 *
...
export const NotesPanel = memo(function NotesPanel({
  ...
  const overflowAbove = rowCount === 0 ? 0 : overflow.above;
  const overflowBelow = rowCount === 0 ? 0 : overflow.below;
  ...
  const listBody = (() => {
    ...
    return (
      <Ledger
        ...
        rowKey={(row) => row.name}
        onWindow={(win) => setOverflow({ above: win.start, below: rowCount - win.end })}
      />
    );
  })();
