/**
 * TreeSurface — Git Tree swimlane DAG (fixture-friendly pane contract).
 *
...
export interface TreeSurfaceLane {
...
export interface TreeSurfaceData {
  readonly ruler: string;
  readonly lanes: readonly TreeSurfaceLane[];
...
export function allocateLaneColumns(
  innerW: number,
  lanes: readonly TreeSurfaceLane[],
...
export function layout(
  ...
  const placeholderLanes: TreeSurfaceLane[] = Array.from({ length: laneCount }, (_, index) => ({
    branch: `lane-${index}`,
    rail: '○━▶',
    color: '#ffffff',
  }));
...
export const TreeSurface = memo(function TreeSurface({
  width,
  height,
  focused,
  theme,
  data,
}: TreeSurfaceProps): React.JSX.Element {
  ...
  return (
    ...
    >
      <TransitBody data={data} width={width} height={height} theme={theme} />
    </Pane>
  );
});
