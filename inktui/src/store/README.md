...
command path — the sole caller of `ApplicationClient` for RPC. High-rate read **streams** may subscribe to
the bus directly (via `useApplicationClient`); the only such case today is tmux frames (`TmuxMode`), kept
