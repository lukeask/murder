"""Stdio JSONL connection to ``codex app-server``.
...
DEFAULT_ARGV: tuple[str, ...] = ("codex", "app-server", "--listen", "stdio://")
...
class AppServerTransport(Protocol):
    """Minimal duplex line transport used by :class:`AppServerConnection`."""
...
class _ProcessTransport:
    def __init__(self, process: asyncio.subprocess.Process) -> None:
        if process.stdin is None or process.stdout is None:
            raise ValueError("app-server process must expose stdin and stdout pipes")
...
class AppServerConnection:
    ...
    def __init__(
        ...
        *,
        ...
        self._argv = tuple(argv) if argv is not None else DEFAULT_ARGV
    ...
    async def respond(
        ...
        *,
        ...
        """Answer a server→client request using the original request id."""
        self._ensure_started()
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.warning("failed to decode app-server line: %r", line, exc_info=True)
                    continue
    ...
    def _route_message(self, message: RpcMessage) -> None:
        if isinstance(message, RpcResponse):
            ...
            if future is None or future.done():
                logger.debug("dropping unmatched app-server response id=%r", message.id)
                return
            if message.error is not None:
                future.set_exception(AppServerRpcError(message.error, request_id=message.id))
            else:
