"""Persistence for the plans and plan_revisions tables."""
# ruff: noqa: E501

from __future__ import annotations

import json
from datetime import datetime
from typing import Any

from murder.state.persistence.connection import RepoDb
from murder.work.plans.schema import Plan


def _now() -> str:
    return datetime.utcnow().isoformat(timespec="seconds")


def insert_plan_revision(
    db: RepoDb,
    plan: Plan,
    *,
    source: str,
    content_hash: str,
) -> int:
    cur = db.conn.execute(
        """
        INSERT INTO plan_revisions
            (repository_id, plan_name, created_at, source, status, body, frontmatter_json, content_hash)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            db.repository_id,
            plan.name,
            _now(),
            source,
            plan.status.value,
            plan.body,
            json.dumps(plan.frontmatter, sort_keys=True, default=str),
            content_hash,
        ),
    )
    db.conn.execute(
        "UPDATE plans SET revision_count = revision_count + 1 WHERE repository_id = ? AND name = ?",
        (db.repository_id, plan.name),
    )
    return int(cur.lastrowid or 0)


def upsert_plan(
    db: RepoDb,
    plan: Plan,
    *,
    content_hash: str,
    materialized_path: str,
    file_hash: str | None = None,
    sync_state: str = "synced",
    parse_error: str | None = None,
    create_revision: bool = True,
    revision_source: str = "db",
) -> None:
    now = _now()
    existing = db.conn.execute(
        "SELECT revision_count FROM plans WHERE repository_id = ? AND name = ?",
        (db.repository_id, plan.name),
    ).fetchone()
    if existing is None:
        db.conn.execute(
            """
            INSERT INTO plans
                (repository_id, name, status, created_at, updated_at, body, frontmatter_json,
                 body_hash, file_hash, materialized_path, sync_state, parse_error)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                db.repository_id,
                plan.name,
                plan.status.value,
                plan.created_at.isoformat(timespec="seconds"),
                plan.updated_at.isoformat(timespec="seconds") if plan.updated_at else now,
                plan.body,
                json.dumps(plan.frontmatter, sort_keys=True, default=str),
                content_hash,
                file_hash,
                materialized_path,
                sync_state,
                parse_error,
            ),
        )
    else:
        db.conn.execute(
            """
            UPDATE plans
               SET status = ?, updated_at = ?, body = ?, frontmatter_json = ?,
                   body_hash = ?, file_hash = ?, materialized_path = ?,
                   sync_state = ?, parse_error = ?
             WHERE repository_id = ? AND name = ?
            """,
            (
                plan.status.value,
                plan.updated_at.isoformat(timespec="seconds") if plan.updated_at else now,
                plan.body,
                json.dumps(plan.frontmatter, sort_keys=True, default=str),
                content_hash,
                file_hash,
                materialized_path,
                sync_state,
                parse_error,
                db.repository_id,
                plan.name,
            ),
        )
    db.conn.execute(
        "DELETE FROM plan_related_tickets WHERE repository_id = ? AND plan_name = ?",
        (db.repository_id, plan.name),
    )
    for ticket_id in plan.related_tickets:
        db.conn.execute(
            """
            INSERT OR IGNORE INTO plan_related_tickets(repository_id, plan_name, ticket_id)
            VALUES (?, ?, ?)
            """,
            (db.repository_id, plan.name, ticket_id),
        )
    if create_revision:
        insert_plan_revision(db, plan, source=revision_source, content_hash=content_hash)


def list_plans(db: RepoDb) -> list[dict[str, Any]]:
    rows = db.conn.execute(
        """
        SELECT p.*,
               (SELECT COUNT(*) FROM plan_revisions r WHERE r.repository_id = p.repository_id AND r.plan_name = p.name) AS revisions
          FROM plans p
         WHERE p.repository_id = ? AND p.status != 'superseded'
         ORDER BY COALESCE(
           (SELECT MAX(captured_at) FROM agent_messages
             WHERE repository_id = p.repository_id AND agent_id = 'planner-' || p.name
               AND role IN ('user', 'assistant')),
           p.created_at
         ) DESC, p.name
        """,
        (db.repository_id,),
    ).fetchall()
    return [dict(r) for r in rows]


def get_plan_row(db: RepoDb, name: str) -> dict[str, Any] | None:
    row = db.conn.execute(
        "SELECT * FROM plans WHERE repository_id = ? AND name = ?", (db.repository_id, name)
    ).fetchone()
    return dict(row) if row else None


# A plan is "live" (and so owns its name) unless it has been superseded.
# Mirrors the notes precedent (``active_note_name_exists`` treats only
# ``status == 'active'`` rows as name-owning). The not-live plan status is
# ``superseded`` (set by ``deprecate_plan``). ``draft`` and ``accepted`` are live.
_LIVE_PLAN_STATUSES = ("draft", "accepted")


def live_plan_name_exists(db: RepoDb, name: str) -> bool:
    """True iff a *live* (non-superseded) plan already owns ``name``.

    Status-aware existence guard for ``plan.create`` (the plans analogue of
    notes' ``active_note_name_exists``): a superseded plan does not block reuse
    of its name, but a live plan does — creating over a live name would clobber
    its body via the scaffold upsert, the data-integrity violation we forbid.
    """
    row = get_plan_row(db, name)
    return row is not None and str(row.get("status", "")) in _LIVE_PLAN_STATUSES


def get_plan_row_by_materialized_path(
    db: RepoDb,
    materialized_path: str,
) -> dict[str, Any] | None:
    row = db.conn.execute(
        "SELECT * FROM plans WHERE repository_id = ? AND materialized_path = ?",
        (db.repository_id, materialized_path),
    ).fetchone()
    return dict(row) if row else None


def rename_plan(
    db: RepoDb,
    old_name: str,
    new_name: str,
    *,
    materialized_path: str,
) -> dict[str, Any]:
    """Move a plan primary key and child references to ``new_name``.

    The schema's child tables reference ``plans(name)`` without ON UPDATE
    CASCADE, so this copies the parent row to the new key before moving child
    rows and deleting the old key.
    """
    old_row = get_plan_row(db, old_name)
    if old_row is None:
        raise KeyError(old_name)
    if get_plan_row(db, new_name) is not None:
        raise ValueError(f"plan already exists: {new_name}")

    now = _now()
    db.conn.execute(
        """
        INSERT INTO plans
            (repository_id, name, status, created_at, updated_at, body, frontmatter_json,
             body_hash, file_hash, materialized_path, sync_state, parse_error,
             revision_count)
        SELECT ?, ?, status, created_at, ?, body, frontmatter_json,
               body_hash, file_hash, ?, sync_state, parse_error, revision_count
          FROM plans
         WHERE repository_id = ? AND name = ?
        """,
        (db.repository_id, new_name, now, materialized_path, db.repository_id, old_name),
    )
    db.conn.execute(
        "UPDATE plan_revisions SET plan_name = ? WHERE repository_id = ? AND plan_name = ?",
        (new_name, db.repository_id, old_name),
    )
    db.conn.execute(
        "UPDATE plan_related_tickets SET plan_name = ? WHERE repository_id = ? AND plan_name = ?",
        (new_name, db.repository_id, old_name),
    )
    has_plan_tickets = db.conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'plan_tickets'"
    ).fetchone()
    if has_plan_tickets is not None:
        db.conn.execute(
            "UPDATE plan_tickets SET plan_name = ? WHERE repository_id = ? AND plan_name = ?",
            (new_name, db.repository_id, old_name),
        )
    db.conn.execute(
        "DELETE FROM plans WHERE repository_id = ? AND name = ?", (db.repository_id, old_name)
    )
    return get_plan_row(db, new_name) or {}


def deprecate_plan(
    db: RepoDb,
    name: str,
    *,
    materialized_path: str,
    file_hash: str,
    body_hash: str,
    body: str,
    frontmatter_json: str,
) -> dict[str, Any]:
    now = _now()
    cur = db.conn.execute(
        """
        UPDATE plans
           SET status = 'superseded', updated_at = ?, body = ?,
               frontmatter_json = ?, body_hash = ?, file_hash = ?,
               materialized_path = ?, sync_state = 'synced', parse_error = NULL
         WHERE repository_id = ? AND name = ?
        """,
        (
            now,
            body,
            frontmatter_json,
            body_hash,
            file_hash,
            materialized_path,
            db.repository_id,
            name,
        ),
    )
    if cur.rowcount == 0:
        raise KeyError(name)
    return get_plan_row(db, name) or {}


def mark_plan_sync_state(
    db: RepoDb,
    name: str,
    sync_state: str,
    *,
    file_hash: str | None = None,
    parse_error: str | None = None,
) -> None:
    db.conn.execute(
        """
        UPDATE plans
           SET sync_state = ?, file_hash = COALESCE(?, file_hash),
               parse_error = ?, updated_at = ?
         WHERE repository_id = ? AND name = ?
        """,
        (sync_state, file_hash, parse_error, _now(), db.repository_id, name),
    )
