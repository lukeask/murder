...
def get_note(db: RepoDb, name: str) -> dict[str, Any] | None:
    return get_doc(db, _TABLE, name)


def list_notes(db: RepoDb) -> list[dict[str, Any]]:
    return list_docs(db, _TABLE)


def latest_note_name(db: RepoDb) -> str | None:
    return latest_doc_name(db, _TABLE)


def upsert_note(db: RepoDb, name: str, *, body: str, materialized_path: str) -> None:
    upsert_doc(db, _TABLE, name, body=body, materialized_path=materialized_path)


def rename_note(db: RepoDb, old_name: str, new_name: str, *, materialized_path: str) -> None:
    rename_doc(
        db,
        _TABLE,
        _REVISIONS_TABLE,
...
def mark_note_retired(db: RepoDb, name: str, *, materialized_path: str) -> None:
...
def list_note_revisions(db: RepoDb, name: str) -> list[dict[str, Any]]:
