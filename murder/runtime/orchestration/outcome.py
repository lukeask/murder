...
class TicketOutcomeService:
    ...
    async def fail_ticket(self, ticket_id: str, reason: str) -> None:
        old = ticket_db.get_ticket_status(self.db, ticket_id)
        prev = TicketStatus(old) if old else TicketStatus.IN_PROGRESS
        try:
            prev = lifecycle.transition(self.db, ticket_id, TicketStatus.FAILED)
        except Exception:
            ticket_db.update_ticket_status(self.db, ticket_id, TicketStatus.FAILED.value)
        lifecycle.set_last_error(self.db, ticket_id, reason)
    ...
    async def complete_ticket(self, ticket_id: str) -> None:
        ...
        """
        status = ticket_db.get_ticket_status(self.db, ticket_id)
        ...
        if status == TicketStatus.PLANNED.value:
            lifecycle.transition(self.db, ticket_id, TicketStatus.READY, reason="completion")
            status = TicketStatus.READY.value
        if status in (TicketStatus.READY.value, TicketStatus.BLOCKED.value):
            lifecycle.transition(
                self.db, ticket_id, TicketStatus.IN_PROGRESS, reason="completion"
            )
        prev = lifecycle.transition(self.db, ticket_id, TicketStatus.DONE)
    ...
    async def block_ticket(self, ticket_id: str, reason: str) -> None:
        ticket_db.update_ticket_status(self.db, ticket_id, TicketStatus.BLOCKED.value)
        await self.escalations.record_ticket_failure(ticket_id, reason)
    ...
    async def _prune_terminal_worktree(self, ticket_id: str) -> None:
        ...
        try:
            await prune_terminal_crow_worktree(self.db, self.repo_root, ticket_id)
