...
from murder.app.tui.note_capture import RECENT_NOTE_ROWS, NoteCaptureScreen
from murder.app.tui.pane_capture import CapturePaneFn
from murder.app.tui.pane_mirror import PaneMirror
from murder.app.tui.perf_log import make_perf_log
...
from murder.app.tui.settings_screen import SettingsScreen
from murder.app.tui.spawn_wizard import SpawnWizard, build_worktree_options
...
class MurderApp(App[None]):
    ...
    def _directional_focus_target(
        self,
        direction: str,
        candidates: list[Widget],
    ) -> Widget | None:
        current = next((pane for pane in candidates if self._focus_contains(pane)), None)
        if current is None:
            return None
        ...
        for index, candidate in enumerate(candidates):
            ...
            if direction == "right":
                primary_gap = region.x - source.right
                overlap = self._axis_overlap(source.y, source.bottom, region.y, region.bottom)
                secondary_gap = (
                    0
                    if overlap > 0
                    else min(abs(source.y - region.bottom), abs(region.y - source.bottom))
                )
            elif direction == "left":
                primary_gap = source.x - region.right
                overlap = self._axis_overlap(source.y, source.bottom, region.y, region.bottom)
                secondary_gap = (
                    0
                    if overlap > 0
                ...
                primary_gap = region.y - source.bottom
                overlap = self._axis_overlap(source.x, source.right, region.x, region.right)
                secondary_gap = (
                    0
                    if overlap > 0
                    else min(abs(source.x - region.right), abs(region.x - source.right))
                )
            else:  # up
                primary_gap = source.y - region.bottom
                overlap = self._axis_overlap(source.x, source.right, region.x, region.right)
                secondary_gap = (
                    0
                    if overlap > 0
    ...
    def _shift_crows_focus(self, direction: str) -> bool:
        candidates = [
            ...
            if self._is_displayed(widget)
        ]
        return self._focus_target(self._directional_focus_target(direction, candidates))
