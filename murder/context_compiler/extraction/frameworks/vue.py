"""Vue single-file component extractor.

``.vue`` files are not valid JS/TS, so this module provides a dedicated
:class:`LanguageExtractor` that:

* emits one file-scoped component aggregate (``language_kind=file``,
  ``semantic_role=component``);
* extracts child units from ``<script>`` / ``<script setup>`` via the JS/TS
  tree-sitter backend;
* records template/style facets and external ``src=`` resource links;
* derives ``renders_component`` from template tags that match imports.
"""

from __future__ import annotations

import re
from dataclasses import replace
from pathlib import PurePosixPath

from murder.context_compiler.extraction.common import (
    DiagnosticAccumulator,
    derive_contains_relationships,
    make_local_id,
)
from murder.context_compiler.extraction.frameworks._sfc import (
    SfcBlock,
    component_name_from_path,
    pascal_to_kebab,
    shift_line,
    split_sfc_blocks,
)
from murder.context_compiler.extraction.models import (
    IMPORT_RESOURCE,
    REL_RENDERS_COMPONENT,
    REL_STYLE_OF,
    REL_TEMPLATE_OF,
    RESOURCE_ASSET,
    RESOURCE_STYLE,
    RESOURCE_TEMPLATE,
    ExtractedImport,
    ExtractedReference,
    ExtractedRelationship,
    ExtractedResourceLink,
    ExtractedSemanticUnit,
    ExtractionDiagnostic,
    FileExtraction,
    ParseStatus,
)
from murder.context_compiler.extraction.registry import (
    LANG_VUE,
    ExtractorRegistry,
)
from murder.context_compiler.extraction.treesitter.base import (
    GRAMMAR_JAVASCRIPT,
    GRAMMAR_TSX,
    GRAMMAR_TYPESCRIPT,
    grammar_status,
    parse_source,
)
from murder.context_compiler.extraction.treesitter.javascript import extract_tree

EXTRACTOR_ID = "vue-sfc"
EXTRACTOR_VERSION = "vue-sfc-1"

_TEMPLATE_TAG_RE = re.compile(
    r"<(?P<name>[A-Z][A-Za-z0-9]*|[a-z][a-z0-9]*(?:-[a-z0-9]+)+)(?=[\s/>])"
)
_DEFINE_PROPS_RE = re.compile(r"\bdefineProps\s*(?:<[^>]*>)?\s*\(")
_DEFINE_EMITS_RE = re.compile(r"\bdefineEmits\s*(?:<[^>]*>)?\s*\(")
_DEFINE_EXPOSE_RE = re.compile(r"\bdefineExpose\s*\(")


def _path_extension(path: str) -> str:
    name = PurePosixPath(path.replace("\\", "/")).name
    if name.startswith(".") and name.count(".") == 1:
        return ""
    return PurePosixPath(name).suffix.lower()


def _line_count(source: str) -> int:
    if not source:
        return 1
    return source.count("\n") + (0 if source.endswith("\n") else 1)


def _script_language(block: SfcBlock) -> tuple[str, str]:
    """Return ``(language_key, grammar_key)`` for a script block."""
    lang = (block.attrs.get("lang") or "").strip().lower()
    if lang in {"ts", "typescript"}:
        return "typescript", GRAMMAR_TYPESCRIPT
    if lang in {"tsx"}:
        return "tsx", GRAMMAR_TSX
    if lang in {"jsx"}:
        return "jsx", GRAMMAR_JAVASCRIPT
    return "javascript", GRAMMAR_JAVASCRIPT


def _shift_extraction(
    extraction: FileExtraction,
    *,
    line_offset: int,
    parent_local_id: str,
    id_prefix: str,
) -> FileExtraction:
    """Rebase script extraction lines and nest units under the SFC component."""
    id_map: dict[str, str] = {}
    units: list[ExtractedSemanticUnit] = []
    for unit in extraction.semantic_units:
        new_id = f"{id_prefix}/{unit.local_id}"
        id_map[unit.local_id] = new_id
        parent = (
            parent_local_id
            if unit.parent_local_id is None
            else id_map.get(unit.parent_local_id, f"{id_prefix}/{unit.parent_local_id}")
        )
        # Parent map may not yet contain parent if ordering is wrong — fix in second pass.
        units.append(
            replace(
                unit,
                local_id=new_id,
                parent_local_id=parent,
                start_line=unit.start_line + line_offset,
                end_line=unit.end_line + line_offset,
            )
        )

    # Second pass: fix parents that were prefixed before the parent was remapped.
    fixed_units: list[ExtractedSemanticUnit] = []
    for unit in units:
        resolved_parent: str | None = unit.parent_local_id
        if resolved_parent and resolved_parent.startswith(f"{id_prefix}/"):
            # already remapped or synthetic
            original = resolved_parent[len(id_prefix) + 1 :]
            if original in id_map:
                resolved_parent = id_map[original]
            fixed_units.append(replace(unit, parent_local_id=resolved_parent))
        else:
            fixed_units.append(unit)

    def remap(value: str | None) -> str | None:
        if value is None:
            return None
        return id_map.get(value, value)

    imports = tuple(
        replace(
            item,
            source_unit_local_id=remap(item.source_unit_local_id) or parent_local_id,
            start_line=item.start_line + line_offset,
            end_line=item.end_line + line_offset,
        )
        for item in extraction.imports
    )
    references = tuple(
        replace(
            item,
            source_unit_local_id=remap(item.source_unit_local_id) or parent_local_id,
            candidate_local_ids=tuple(remap(x) or x for x in item.candidate_local_ids),
            start_line=item.start_line + line_offset,
            end_line=item.end_line + line_offset,
        )
        for item in extraction.references
    )
    relationships = tuple(
        replace(
            item,
            source_unit_local_id=remap(item.source_unit_local_id) or parent_local_id,
            target_local_id=remap(item.target_local_id),
            start_line=shift_line(item.start_line, line_offset),
            end_line=shift_line(item.end_line, line_offset),
        )
        for item in extraction.relationships
    )
    return replace(
        extraction,
        semantic_units=tuple(fixed_units),
        imports=imports,
        references=references,
        relationships=relationships,
    )


def _imported_component_aliases(imports: tuple[ExtractedImport, ...]) -> dict[str, ExtractedImport]:
    """Map local aliases / names that look like components to their import."""
    out: dict[str, ExtractedImport] = {}
    for item in imports:
        alias = item.local_alias or item.imported_name
        if not alias:
            continue
        # Vue SFC / component modules, or PascalCase named imports.
        spec = item.module_specifier
        is_componentish = spec.endswith(".vue") or bool(re.match(r"^[A-Z]", alias)) or "/" in spec
        if not is_componentish:
            continue
        out[alias] = item
        out[pascal_to_kebab(alias)] = item
    return out


def _template_component_tags(template: str) -> list[tuple[str, int]]:
    found: list[tuple[str, int]] = []
    for match in _TEMPLATE_TAG_RE.finditer(template):
        name = match.group("name")
        line = template[: match.start()].count("\n") + 1
        found.append((name, line))
    return found


def _resolve_src(path: str, src: str) -> str:
    base_dir = PurePosixPath(path.replace("\\", "/")).parent
    joined = (base_dir / src).as_posix()
    parts: list[str] = []
    for part in joined.split("/"):
        if part in ("", "."):
            continue
        if part == "..":
            if parts:
                parts.pop()
            continue
        parts.append(part)
    return "/".join(parts)


class VueExtractor:
    """Language extractor for Vue ``.vue`` single-file components."""

    extractor_id = EXTRACTOR_ID
    extractor_version = EXTRACTOR_VERSION

    def supports(self, path: str, language_hint: str | None = None) -> bool:
        if _path_extension(path) == ".vue":
            return True
        if language_hint and language_hint.strip().lower() in {"vue"}:
            return True
        return False

    def extract(  # noqa: PLR0912, PLR0915
        self,
        path: str,
        source: str,
        *,
        language_hint: str | None = None,
    ) -> FileExtraction:
        diagnostics = DiagnosticAccumulator(self.extractor_id)
        name = component_name_from_path(path)
        total_lines = max(_line_count(source), 1)
        component_id = make_local_id(language_kind="file", qualified_name=name)
        component = ExtractedSemanticUnit(
            local_id=component_id,
            language_kind="file",
            semantic_role="component",
            qualified_name=name,
            unqualified_name=name,
            start_line=1,
            end_line=total_lines,
            exported=True,
            metadata={"framework": "vue", "sfc": True},
        )

        blocks = split_sfc_blocks(source)
        script_blocks = [b for b in blocks if b.name == "script"]
        template_blocks = [b for b in blocks if b.name == "template"]
        style_blocks = [b for b in blocks if b.name == "style"]

        units: list[ExtractedSemanticUnit] = [component]
        imports: list[ExtractedImport] = []
        references: list[ExtractedReference] = []
        relationships: list[ExtractedRelationship] = []
        resource_links: list[ExtractedResourceLink] = []
        extra_diags: list[ExtractionDiagnostic] = []
        parse_status: ParseStatus = "parsed"
        setup_meta: dict[str, object] = {}

        for index, block in enumerate(script_blocks):
            if "setup" in block.attrs:
                setup_meta["script_setup"] = True
            src = block.attrs.get("src")
            if src:
                target = _resolve_src(path, src)
                resource_links.append(
                    ExtractedResourceLink(
                        source_unit_local_id=component_id,
                        target_path=target,
                        resource_kind=RESOURCE_ASSET,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        metadata={"block": "script", "attr": "src", "framework": "vue"},
                    )
                )
                imports.append(
                    ExtractedImport(
                        source_unit_local_id=component_id,
                        module_specifier=src,
                        import_kind=IMPORT_RESOURCE,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        metadata={"block": "script", "external_src": True},
                    )
                )
                continue

            language, grammar = _script_language(block)
            status = grammar_status(grammar)
            if not status.available and grammar == GRAMMAR_TYPESCRIPT:
                grammar = GRAMMAR_JAVASCRIPT
                language = "javascript"
                status = grammar_status(grammar)
            if not status.available:
                diagnostics.warning(
                    f"script grammar {grammar!r} unavailable; skipping script structure",
                    start_line=block.open_line,
                    code="grammar_unavailable",
                )
                parse_status = "partial"
                continue

            try:
                tree = parse_source(grammar, block.content)
            except Exception as exc:
                diagnostics.warning(
                    f"script parse failed: {type(exc).__name__}: {exc}",
                    start_line=block.open_line,
                    code="script_parse_failed",
                )
                parse_status = "partial"
                continue
            if tree is None:
                diagnostics.warning(
                    "script parse unavailable",
                    start_line=block.open_line,
                    code="grammar_unavailable",
                )
                parse_status = "partial"
                continue

            script_extraction = extract_tree(
                path=path,
                language=language,
                backend=f"{self.extractor_id}:script",
                tree=tree,
            )
            if script_extraction.parse_status in {"partial", "failed"}:
                parse_status = "partial"
            line_offset = block.content_start_line - 1
            shifted = _shift_extraction(
                script_extraction,
                line_offset=line_offset,
                parent_local_id=component_id,
                id_prefix=f"script{index}",
            )
            units.extend(shifted.semantic_units)
            imports.extend(shifted.imports)
            references.extend(shifted.references)
            relationships.extend(shifted.relationships)
            extra_diags.extend(shifted.diagnostics)

            # defineProps / defineEmits / defineExpose markers.
            for pattern, key in (
                (_DEFINE_PROPS_RE, "defineProps"),
                (_DEFINE_EMITS_RE, "defineEmits"),
                (_DEFINE_EXPOSE_RE, "defineExpose"),
            ):
                match = pattern.search(block.content)
                if match:
                    setup_meta[key] = True
                    line = block.content_start_line + block.content[: match.start()].count("\n")
                    references.append(
                        ExtractedReference(
                            source_unit_local_id=component_id,
                            identifier=key,
                            reference_kind="macro",
                            start_line=line,
                            end_line=line,
                            resolution_method="vue_macro",
                            metadata={"framework": "vue"},
                        )
                    )

        # Merge setup/macro metadata onto the component unit.
        if setup_meta:
            units[0] = replace(
                component,
                metadata={**dict(component.metadata), **setup_meta},
            )

        # Template blocks.
        component_aliases = _imported_component_aliases(tuple(imports))
        for index, block in enumerate(template_blocks):
            src = block.attrs.get("src")
            if src:
                target = _resolve_src(path, src)
                resource_links.append(
                    ExtractedResourceLink(
                        source_unit_local_id=component_id,
                        target_path=target,
                        resource_kind=RESOURCE_TEMPLATE,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        metadata={"block": "template", "attr": "src", "framework": "vue"},
                    )
                )
                relationships.append(
                    ExtractedRelationship(
                        source_unit_local_id=component_id,
                        target_path=target,
                        relation_kind=REL_TEMPLATE_OF,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        confidence=1.0,
                        resolution_method="vue_sfc_src",
                    )
                )
                continue

            # Inline template is a facet of the component (resource link to self path
            # with metadata), not a separate top-level component symbol.
            resource_links.append(
                ExtractedResourceLink(
                    source_unit_local_id=component_id,
                    target_path=path,
                    resource_kind=RESOURCE_TEMPLATE,
                    start_line=block.start_line,
                    end_line=block.end_line,
                    metadata={
                        "block": "template",
                        "inline": True,
                        "framework": "vue",
                        "index": index,
                    },
                )
            )
            for tag, rel_line in _template_component_tags(block.content):
                abs_line = block.content_start_line + rel_line - 1
                imported = component_aliases.get(tag)
                target_name = (
                    (imported.local_alias or imported.imported_name or tag) if imported else tag
                )
                # Only emit for imported/PascalCase component tags.
                if imported is None and not re.match(r"^[A-Z]", tag):
                    continue
                relationships.append(
                    ExtractedRelationship(
                        source_unit_local_id=component_id,
                        target_qualified_name=target_name,
                        target_path=imported.module_specifier if imported else None,
                        relation_kind=REL_RENDERS_COMPONENT,
                        start_line=abs_line,
                        end_line=abs_line,
                        confidence=0.8 if imported else 0.55,
                        resolution_method="vue_template_tag",
                        metadata={"tag": tag, "framework": "vue"},
                    )
                )

        # Style blocks.
        for index, block in enumerate(style_blocks):
            src = block.attrs.get("src")
            scoped = "scoped" in block.attrs
            module = "module" in block.attrs
            if src:
                target = _resolve_src(path, src)
                resource_links.append(
                    ExtractedResourceLink(
                        source_unit_local_id=component_id,
                        target_path=target,
                        resource_kind=RESOURCE_STYLE,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        metadata={
                            "block": "style",
                            "attr": "src",
                            "scoped": scoped,
                            "module": module,
                            "framework": "vue",
                        },
                    )
                )
                relationships.append(
                    ExtractedRelationship(
                        source_unit_local_id=component_id,
                        target_path=target,
                        relation_kind=REL_STYLE_OF,
                        start_line=block.open_line,
                        end_line=block.open_line,
                        confidence=1.0,
                        resolution_method="vue_sfc_src",
                        metadata={"scoped": scoped, "module": module},
                    )
                )
            else:
                resource_links.append(
                    ExtractedResourceLink(
                        source_unit_local_id=component_id,
                        target_path=path,
                        resource_kind=RESOURCE_STYLE,
                        start_line=block.start_line,
                        end_line=block.end_line,
                        metadata={
                            "block": "style",
                            "inline": True,
                            "scoped": scoped,
                            "module": module,
                            "framework": "vue",
                            "index": index,
                            "lang": block.attrs.get("lang") or "css",
                        },
                    )
                )
                relationships.append(
                    ExtractedRelationship(
                        source_unit_local_id=component_id,
                        target_path=path,
                        relation_kind=REL_STYLE_OF,
                        start_line=block.start_line,
                        end_line=block.end_line,
                        confidence=1.0,
                        resolution_method="vue_sfc_inline_style",
                        metadata={"scoped": scoped, "module": module, "inline": True},
                    )
                )

        contains = derive_contains_relationships(units)
        relationships_out = tuple(
            sorted(
                (*relationships, *contains),
                key=lambda r: (
                    r.relation_kind,
                    r.source_unit_local_id or "",
                    r.target_local_id or "",
                    r.target_qualified_name or "",
                    r.target_path or "",
                    r.start_line or 0,
                ),
            )
        )
        if not script_blocks and not template_blocks:
            diagnostics.info("vue SFC has no script/template blocks", code="empty_sfc")
            parse_status = "partial"

        return FileExtraction(
            path=path,
            language=LANG_VUE,
            parse_status=parse_status,
            semantic_units=tuple(units),
            imports=tuple(
                sorted(
                    imports,
                    key=lambda i: (
                        i.start_line,
                        i.import_kind,
                        i.module_specifier,
                        i.imported_name or "",
                    ),
                )
            ),
            references=tuple(
                sorted(
                    references,
                    key=lambda r: (r.start_line, r.reference_kind, r.identifier),
                )
            ),
            relationships=relationships_out,
            resource_links=tuple(
                sorted(
                    resource_links,
                    key=lambda r: (
                        r.resource_kind,
                        r.source_unit_local_id,
                        r.target_path,
                        r.start_line or 0,
                    ),
                )
            ),
            diagnostics=tuple([*diagnostics.as_tuple(), *extra_diags]),
        )


def register_vue_extractor(registry: ExtractorRegistry) -> None:
    """Register the Vue SFC extractor for ``.vue`` files."""
    registry.register(
        VueExtractor(),
        languages=(LANG_VUE,),
        extensions=(".vue",),
        priority=30,
    )


__all__ = [
    "EXTRACTOR_ID",
    "EXTRACTOR_VERSION",
    "VueExtractor",
    "register_vue_extractor",
]
