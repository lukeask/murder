...
if TYPE_CHECKING:
    from murder_newstructure.escalations.service import EscalationService
...
class TicketOutcomeService:
    ...
    async def fail_ticket(self, ticket_id: str, reason: str) -> None:
        old = dbmod.get_ticket_status(self.conn, ticket_id)
        ...
        try:
            ...
            dbmod.update_ticket_status(self.conn, ticket_id, TicketStatus.FAILED.value)
    ...
    async def block_ticket(self, ticket_id: str, reason: str, *, path: str | None = None) -> None:
        if path is not None:
            ...
            return
        dbmod.update_ticket_status(self.conn, ticket_id, TicketStatus.BLOCKED.value)
        await self.escalations.record_ticket_failure(ticket_id, reason)
    ...
    async def complete_after_crow(
        ...
        *,
        ...
        row = dbmod.get_ticket(self.conn, ticket_id)
        if row is None:
            return False
