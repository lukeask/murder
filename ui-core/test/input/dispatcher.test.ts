...
describe('layer 0 — active-mode capture', () => {
  ...
  function captureMode(passThrough = false): TransientMode {
    return {
      passThrough,
      keymap: [
  ...
  it('calls onUncaptured for unmatched keys when present — returns handled:true if consumed', () => {
    ...
    const modeWithUncaptured: TransientMode = {
      ...captureMode(),
      onUncaptured(input) {
...
describe('layer 1 — global chords', () => {
  ...
  it('alt+g opens the workflow template editor', () => {
    const h = handlers();
    const out = dispatchKey('g', makeKey({ meta: true }), ctx('plans', h));
    expect(h.openWorkflowTemplateEditor).toHaveBeenCalledOnce();
    expect(out).toEqual({
      layer: 'global',
      handled: true,
